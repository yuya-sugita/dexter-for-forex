# Renaissance（Polymarket 移植版）

Renaissance（旧称 Sapiens、元は FX/CFD 向け）の **Polymarket 予測市場
向け移植版**ブランチです。本ブランチ (`claude/polymarket-port`) では
対象資産が予測市場のバイナリコントラクト（YES/NO、価格 0〜1、満期で
0 or 1 に確定）に変わります。統計分析、ベイズ更新、モンテカルロ
シミュレーション、ケリー基準によるバンクロール管理など、クオンツ
レベルの分析をターミナル上で実行します。
...
> と**ツール表面**だけです。むしろ予測市場は、ベイズ更新（天文学者）・
> イベント駆動の相転移（物理学者）・情報漏洩（暗号解読者）といった
> 異邦人の武器が直接適用できる、Renaissance 哲学のより自然な土壌
> です。

思想の核は `SOUL.md` および `CLAUDE.md` の「思考プロトコル」セクション
（11個）に集約されています。
